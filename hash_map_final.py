...
class HashMap:
    ...
    def resize_table(self, capacity):
        ...
        # check each buckets
        for i in range(self.capacity):
            # set the head of this bucket to target
            target = self._buckets[i].head
            # stop this time of iteration when it reaches the end of the linked list
            while target:
                temp.add_front(target.key, target.value)
                target = target.next
